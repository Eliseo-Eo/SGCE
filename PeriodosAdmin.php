...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['AltaCiclo'])) {
        ...
        if ($Nombre !== '' && preg_match('/^\d{4}-\d{2}-\d{2}$/', $FechaInicio) && preg_match('/^\d{4}-\d{2}-\d{2}$/', $FechaFin)) {
            ...
            $_SESSION['MensajePeriodos'] = 'Ciclo escolar guardado.';
        } else {
    ...
    if (isset($_POST['AltaPeriodo'])) {
        ...
        if ($CicloId > 0 && $Nombre !== '') {
            ...
            $_SESSION['MensajePeriodos'] = 'Periodo guardado.';
        } else {
...
<head>
    ...
    <link rel="stylesheet" href="assets/css/PeriodosAdmin.css?v=45">
</head>
<body>
<div class="SgceModuleWrap">
    <div class="Top mb-4">
        <div>
            <h1><i class="fa-solid fa-calendar-days"></i> Ciclos escolares y periodos</h1>
            <p>Administra el ciclo activo y los periodos para separar calificaciones por evaluación.</p>
        </div>
        <a class="BtnBack" href="Admin.php?Tab=inicio"><i class="fa-solid fa-house"></i> Volver a inicio</a>
    </div>

    <?php if ($Mensaje): ?>
        <div class="alert alert-info fw-bold"><i class="fa-solid fa-circle-info"></i> <?= HPeriodo($Mensaje) ?></div>
    <?php endif; ?>

    <div class="row g-4">
        <div class="col-lg-5">
            <div class="Card p-4 h-100">
                <h5>Nuevo / editar ciclo</h5>
                <form method="POST">
                    <?= CampoCsrf() ?>
                    <input type="hidden" name="AltaCiclo">
                    <label class="form-label">Nombre</label>
                    <input name="Nombre" class="form-control mb-3" placeholder="2026-2027" required>
                    <div class="row">
                        <div class="col-md-6">
                            <label class="form-label">Inicio</label>
                            <input type="date" name="FechaInicio" class="form-control mb-3" required>
                        </div>
                        <div class="col-md-6">
                            <label class="form-label">Fin</label>
                            <input type="date" name="FechaFin" class="form-control mb-3" required>
                        </div>
                    </div>
                    <label class="form-check mb-3">
                        <input type="checkbox" name="Activo" class="form-check-input" checked> Activo
                    </label>
        ...
        <div class="col-lg-7">
            <div class="Card p-4 h-100">
                ...
                <form method="POST" class="row g-3">
                    ...
                    <div class="col-12">
                        <label class="form-check"><input type="checkbox" name="ActivoPeriodo" class="form-check-input" checked> Activo</label>
                    </div>
                    <div class="col-12">
                        <button class="BtnPrimary" type="submit"><i class="fa-solid fa-plus"></i> Guardar periodo</button>
                    </div>
                </form>
            </div>
        </div>
    ...
    <div class="row g-4 mt-1">
        <div class="col-lg-5">
            <div class="Card p-4 SgceCompactTable">
                <h5>Ciclos registrados</h5>
