...
function SgceNormalizarTextoUsuarios($Valor) {
    ...
    if (function_exists('mb_strtoupper')) { return mb_strtoupper($Valor, 'UTF-8'); }
    return strtoupper($Valor);
}

function SgceValidarRolUsuario($Rol, $Roles) {
    return is_string($Rol) && array_key_exists($Rol, $Roles);
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        if ($Accion === 'CrearUsuario') {
            ...
            $Stmt->execute([$Username, $Password, $NombreCompleto, $Rol]);
        ...
        if ($Accion === 'EditarUsuario') {
            ...
            if ($Id <= 0 || $NombreCompleto === '' || $Username === '' || $Password === '' || !SgceValidarRolUsuario($Rol, $Roles)) {
            ...
            $SessionSql = $Activo ? '' : ', SessionToken = NULL';
            $Stmt = $Pdo->prepare("UPDATE Usuarios SET Username = ?, Password = ?, NombreCompleto = ?, Rol = ?, Activo = ? $SessionSql WHERE Id = ?");
            $Stmt->execute([$Username, $Password, $NombreCompleto, $Rol, $Activo, $Id]);
...
$Stmt = $Pdo->prepare("SELECT Id, Username, Password, NombreCompleto, Rol, Activo FROM Usuarios $WhereSql ORDER BY Activo DESC, FIELD(Rol,'admin','director','secretario','coordinador','prefecto','maestro'), NombreCompleto ASC LIMIT $Limit OFFSET $Offset");
$Stmt->execute($Params);
...
<div class="MainWrap SgceModuleWrap SgceUsersPage">
    <div class="TopBar">
        ...
        <div class="d-flex gap-2 flex-wrap">
            <a href="Admin.php?Tab=inicio" class="BtnBack"><i class="fa-solid fa-house"></i> Regresar al inicio</a>
        </div>
    </div>
    ...
    <div class="CardPanel SgceUsersCreateCard">
        ...
        <form method="POST" class="row g-3 SgceUsersCreateForm">
            ...
            <div class="col-lg-2">
                ...
                <input type="text" name="Password" class="form-control" required placeholder="visible">
            </div>
            <div class="col-lg-2">
            ...
            <table class="table align-middle UsuariosTable SgceUsersTable">
                ...
                <?php foreach ($Usuarios as $U): ?>
                    ...
                    <tr>
                        ...
                        <td>
                            <input form="<?= $FormEditar ?>" name="Password" class="form-control form-control-sm PasswordVisible" value="<?= htmlspecialchars($U['Password']) ?>" required>
                        </td>
                        <td>
                            ...
                            <div class="ActionsInline">
                                ...
                                <?php if ((int)$U['Activo'] === 1): ?>
                                    <form method="POST" onsubmit="return confirm('¿Desactivar este usuario?');">
                                        ...
                                        <input type="hidden" name="Id" value="<?= (int)$U['Id'] ?>">
                                        <button type="submit" class="BtnSmall BtnDisable" <?= ((int)$U['Id'] === (int)$UserSession['Id']) ? 'disabled' : '' ?>><i class="fa-solid fa-user-slash"></i> Baja</button>
                                    </form>
                                <?php else: ?>
                                    <form method="POST">
                                        <?= CampoCsrf() ?>
...
<script src="assets/js/sgce-shared.js?v=31"></script>
<script src="assets/js/UsuariosAdmin.js?v=31"></script>
</body>
</html>
